...
class UserService {
    ...
    static async userLogin(user_login) {
        try {
            const user = await database.User.findOne({ where: { phone_number: String(user_login.phone_number) } });
            if (user && user.comparePassword(user_login.password)) {
                return user;
            }
